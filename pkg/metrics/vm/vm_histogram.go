...
// prefix - <metric_name>{<optional_tags>}
func (h *VmHistogram) MarshalTo(name string, labelKey string, w io.Writer) error {
	countTotal := uint64(0)
	h.VisitNonZeroBuckets(func(vmrange string, count uint64) {
		// <metric_name>_bucket{<optional_tags>,vmrange="<start>...<end>"} <counter>
		if labelKey == "" {
			fmt.Fprintf(w, "%s_bucket{vmrange=%q} %d\n", name, vmrange, count)
		} else {
			fmt.Fprintf(w, "%s_bucket{%s,vmrange=%q} %d\n", name, labelKey, vmrange, count)
		}
		countTotal += count
	})
	if countTotal > 0 {
